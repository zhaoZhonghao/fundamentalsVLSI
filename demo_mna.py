#!/usr/bin/env python
"""for Nodal Analysis(NA) and Modified Nodal Analysis(MNA) demo.
...
class Component(object):
    """Component is a class, which has class members for Name, pNode,
       nNode and Value
       ---
       + matrixFill function configures the nodal matrix for each component
    """
...
with open(netlistFile) as netlist:
    ## Second, create a component for each line, and store all
    ## components into a list 'circuit',
    ## where 'circuit' is a whole list of all components.
    ...
    for line in netlist.readlines():
        ...
        ## Ignore comments line, ingore line having less than 4 fields
    ...
    ## However, 0/GND are treated as the same node, not appearing in matirx
    if '0' in nodeSet:
        nodeSet.remove('0')
    if 'GND' in nodeSet:
        nodeSet.remove('GND')
